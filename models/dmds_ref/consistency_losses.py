...
from models.dmds_ref import resampler
from models.dmds_ref import transform_utils
...
def rgbd_consistency_loss(frame1transformed_depth,
  ...
  frame1_closer_to_camera = tf.cast(
      tf.logical_and(
          frame1transformed_depth.mask,
          tf.less(frame1transformed_depth.depth, frame2depth_resampled)), tf.float32)
  ...
  depth_proximity_weight = tf.math.multiply_no_nan(
      ...
      (tf.square(frame2depth_resampled - frame1transformed_depth.depth) +
       depth_error_second_moment), tf.cast(frame1transformed_depth.mask, tf.float32))
  ...
  endpoints = {
      ...
      'warped_images': frame2rgb_resampled
  }
  return endpoints
